...
test_that("test 2: input tibble, bivariate", {
  sm.coxph(formula = Surv(time.true) ~ x + y, data = sjosmooth.tbl)
})

test_that("test 3: input data.frame, univariate", {
  sm.coxph(formula = Surv(time.true) ~ x, data = sjosmooth.df)
})

test_that("test 4: input data.frame, bivariate", {
  sm.coxph(formula = Surv(time.true) ~ x + y, data = sjosmooth.df)
})
