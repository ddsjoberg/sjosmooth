...
newdata = tibble(x = seq(0, 1, by = 0.05),
                 time = 1)
newdata$survival = sm.regress(formula = Surv(time) ~ x, 
                             data = sjosmooth, 
                             lambda = 0.3, 
                             newdata = newdata)
...
newdata = tibble(x = seq(0, 1, by = 0.05),
                 time = 1)
# output is a list, with each element the output for sm.regress for each of the lambda values
est0 = map(lambda, ~ bind_cols(newdata,
                               survival = sm.regress(formula = Surv(time) ~ x, 
                                                    data = sjosmooth, 
                                                    lambda = .x, 
                                                    newdata = newdata)))
```
