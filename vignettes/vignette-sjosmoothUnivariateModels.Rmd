...
sm.surv = sm.timeto(formula = Surv(time) ~ x, 
                    data = sjosmooth, 
                    time = 1, 
                    lambda = 0.2, 
                    grid = tibble(x = seq(0, 1, by = 0.05)))
# plotting the true and estiamted survival functions.
ggplot() + 
  geom_line(data = sm.surv,   aes(y = survival,  x = x)) +
...
# output is a list, with each element the output for sm.timeto for each of the lambda values
est0 = map(lambda, ~ sm.timeto(formula = Surv(time) ~ x, 
                    data = sjosmooth, 
                    time = 1, 
                    lambda = .x, 
                    grid = tibble(x = seq(0, 1, by = 0.05))))
